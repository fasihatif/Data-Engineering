...
```{r}
# Language Detection
language_type_func <- function(x){
  
  type = detect_language(x) %>% pull(LanguageCode)
  return(type)
}

language_type <- data.frame(unlist(lapply(tweetsDf$tweets,language_type_func)))
colnames(language_type) <- "language"

# Visualization for types of languages used

language_type %>%
  group_by(language) %>%
  summarize(count =n()) %>%
  ggplot(aes(x = language, y = count, fill = language, label = count)) +
